...
class Account {
  ...
  void deposit(double amount) {
    ...
    this.display();
    if(this instanceof Curr_Acct){
      ((Curr_Acct)this).penalty();
    }
    else{
      ((Savn_Acct)this).interest();
    }
  }
...
class Savn_Acct extends Account{
  Savn_Acct(String name, int account_no){
...
class Curr_Acct extends Account {
  Curr_Acct(String name, int account_no){
    super(name, account_no);
  }
    ...
    do {
      ...
      switch (sc.nextInt()) {
        case 1:{ System.out.print("\nEnter the name of the Account Holder: ");
              ...
              int inp = sc.nextInt();
              if (inp == 1) {
                acc[i] = new Savn_Acct(n, i);
              }
              else if(inp == 2){
                acc[i] = new Curr_Acct(n, i);
              }
              else{
